...
const Super = {
  ...
  idcard: {
    ...
    /* ============================================================
       ENTERPRISE FINAL V3 (#15): CARD BACK.
       Universal back side for every template family: school header strip,
       card rules, emergency/return info, holder ID + QR (re-scan), barcode,
       authorised signature and issue/expiry dates (dd/mm/yyyy).
       ============================================================ */
    htmlBack(person) {
      const s = Super.school || {};
      person = person || {};
      const pc = person.pc || s.primary || '#1e2a5e';
      const ac = person.ac || s.accent || '#0ea5e9';
      const idNo = person.admission_no || person.staff_no || person.id || '';
      const isStaff = (person.type === 'staff');
      const dmy = (v) => { const d = v ? new Date(v) : new Date(); return String(d.getDate()).padStart(2,'0') + '/' + String(d.getMonth()+1).padStart(2,'0') + '/' + d.getFullYear(); };
      const yr = new Date().getFullYear();
      const issued = dmy(new Date());
      const expires = '31/08/' + (yr + 1);
      const qr = this.qrUrl(JSON.stringify({ id: idNo, name: person.full_name || '', type: person.type || 'student', v: 'back' }), 160);
      const rules = isStaff
        ? ['This card identifies a bona-fide member of staff.','It must be worn visibly within school premises.','It remains the property of the school and must be surrendered on exit.','Report loss to the admin office immediately.']
        : ['This card identifies a bona-fide student of the school.','It must be presented on demand and worn where required.','It is NOT transferable and remains school property.','Report loss to the class teacher or school office immediately.'];
      const logo = 'assets/img/logo.' + (s.logoExt || 'svg');
      const w = (person.template === 'badge') ? 280 : (person.template === 'vertical' ? 300 : (person.template === 'premium' ? 520 : 400));
      return '<div class="sc-idcard-back" style="width:' + w + 'px;border-radius:16px;overflow:hidden;background:#fff;border:1px solid #e2e8f0;font-family:\'Segoe UI\',Arial,sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.12)">' +
        '<div style="background:' + pc + ';color:#fff;padding:8px 14px;display:flex;align-items:center;gap:8px"><img src="' + logo + '" style="width:26px;height:26px;border-radius:6px;background:#fff;padding:2px;object-fit:contain" onerror="this.style.display=\'none\'"><strong style="font-size:.78rem">' + Super.esc(s.name || 'School') + '</strong><span style="margin-left:auto;font-size:.56rem;letter-spacing:2px;opacity:.9">' + (isStaff ? 'STAFF' : 'STUDENT') + ' CARD — BACK</span></div>' +
        '<div style="display:flex;gap:12px;padding:12px 14px">' +
          '<div style="flex:1;min-width:0"><div style="font-size:.62rem;font-weight:800;color:' + pc + ';letter-spacing:1px;margin-bottom:4px">CARD RULES</div>' +
          '<ol style="margin:0;padding-left:14px;font-size:.6rem;color:#334155;line-height:1.6">' + rules.map(r => '<li>' + r + '</li>').join('') + '</ol>' +
          '<div style="font-size:.6rem;color:#334155;margin-top:8px"><b>If found, please return to:</b><br>' + Super.esc(s.name || '') + (s.address ? ', ' + Super.esc(s.address) : '') + (s.phone ? ' · ' + Super.esc(s.phone) : '') + '</div>' +
          '<div style="display:flex;gap:14px;margin-top:8px;font-size:.6rem;color:#334155"><span><b>Issued:</b> ' + issued + '</span><span><b>Expires:</b> ' + expires + '</span></div></div>' +
          '<div style="flex-shrink:0;text-align:center"><img src="' + qr + '" style="width:74px;height:74px" alt="QR"><div style="font-size:.5rem;font-weight:800;color:' + pc + '">RE-SCAN TO VERIFY</div>' +
          '<div style="margin-top:6px">' + this.signBlock('#334155', 80) + '</div></div></div>' +
        '<div style="height:22px;background:repeating-linear-gradient(90deg,#111 0 2px,transparent 2px 5px);margin:0 14px"></div>' +
        '<div style="text-align:center;font-size:.55rem;color:#64748b;padding:4px 0 2px">' + Super.esc(idNo) + '</div>' +
        this.contactStrip(s, false) +
        '<div style="background:#0f172a;color:#94a3b8;font-size:.52rem;text-align:center;padding:3px 0">Powered by HMG Concepts · School Connect</div></div>';
    },
    print(person) {
      const w = window.open('', '_blank');
      w.document.write('<html><head><title>ID Card</title><base href="'+document.baseURI.replace(/[^/]*$/,'')+'"></head><body style="padding:30px"><div style="display:flex;justify-content:center;page-break-after:always">' + this.html(person) + '</div><div style="display:flex;justify-content:center">' + this.htmlBack(person) + '</div>' + '<script>window.onload=function(){var imgs=[].slice.call(document.images),left=imgs.length;if(!left)return window.print();var done=function(){if(--left<=0)setTimeout(function(){window.print()},300)};imgs.forEach(function(im){if(im.complete)done();else{im.onload=done;im.onerror=done}})};<\/script></body></html>');
      w.document.close();
    }
  },
